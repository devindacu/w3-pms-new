...
export function getReservationDetails(reservationId: string, data: CrossModuleData) {
  ...
  const invoice = data.guestInvoices?.find(i => i.reservationId === reservationId)
  ...
  const totalCharges = folio?.lineItems.reduce((sum, item) => sum + item.amount, 0) || 0
  const totalPaid = payments.reduce((sum, p) => sum + p.amount, 0)
...
export function getSupplierDetails(supplierId: string, data: CrossModuleData) {
  ...
  const pendingOrders = purchaseOrders.filter(po => po.status === 'sent' || po.status === 'approved')
  const overdueInvoices = invoices.filter(inv => 
    inv.status === 'pending-payment' && inv.dueDate && inv.dueDate < Date.now()
  )
...
export function getRevenueBreakdown(data: CrossModuleData) {
  const roomRevenue = data.guestInvoices?.reduce((sum, inv) => {
    const roomCharges = inv.lineItems.filter(li => li.type === 'accommodation')
    return sum + roomCharges.reduce((s, li) => s + li.amount, 0)
  }, 0) || 0
  ...
  const extraServicesRevenue = data.guestInvoices?.reduce((sum, inv) => {
    const serviceCharges = inv.lineItems.filter(li => li.type === 'extra-service')
    return sum + serviceCharges.reduce((s, li) => s + li.amount, 0)
  }, 0) || 0
  
  const otherRevenue = data.guestInvoices?.reduce((sum, inv) => {
    const otherCharges = inv.lineItems.filter(li => li.type === 'other')
    return sum + otherCharges.reduce((s, li) => s + li.amount, 0)
  }, 0) || 0
...
export function getChannelManagerStatus(data: CrossModuleData) {
  ...
  const channelBreakdown = connections.map(conn => {
    const connReservations = channelReservations.filter(r => r.channelId === conn.id)
    return {
...
export function getFinancialSummary(data: CrossModuleData) {
  ...
  return {
    ...
    payables: {
      ...
      overdue: overduePayables.reduce((sum, inv) => sum + inv.totalAmount, 0),
    ...
    cashFlow: totalPaid - payments.filter(p => p.type === 'outgoing').reduce((sum, p) => sum + p.amount, 0)
  }
}
...
function calculateOnTimeDeliveryRate(grns: GoodsReceivedNote[], pos: PurchaseOrder[]): number {
  ...
  const onTimeDeliveries = grns.filter(grn => {
    ...
    if (!po || !po.expectedDeliveryDate) return true
    return grn.receivedDate <= po.expectedDeliveryDate
  })
...
export function syncReservationWithRoom(
  ...
  if (reservation.status === 'confirmed' && checkIn <= today && checkOut >= today) {
    setRooms(currentRooms =>
      currentRooms.map(r =>
        r.id === reservation.roomId ? { ...r, status: 'occupied' as const } : r
      )
    )
  } else if (reservation.status === 'checked-out' || reservation.status === 'cancelled') {
    setRooms(currentRooms =>
      currentRooms.map(r =>
        r.id === reservation.roomId ? { ...r, status: 'cleaning' as const } : r
      )
    )
  }
}
...
export function syncInventoryWithConsumption(
  ...
  consumptionLog.items.forEach(item => {
    setFoodItems(currentItems =>
      currentItems.map(fi =>
        fi.id === item.itemId
          ? { ...fi, currentStock: Math.max(0, fi.currentStock - item.quantity) }
          : fi
      )
    )
  })
...
export function createFolioFromReservation(
  ...
  return {
    ...
    folioNumber: `FO-${Date.now().toString().slice(-8)}`,
